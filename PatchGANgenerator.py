...
def format(image_name:str)->None:
    image=transforms_(Image.open(f'flaskApp/images/{image_name}'))
    save_image(image, f"flaskApp/masks/{image_name}", normalize=True)
...
def inpaint(image_name:str , x1:int , y1:int)->None:
    ...
    # Save sample
    save_image(masked_image, f"flaskApp/masks/{image_name}", normalize=True)
    save_image(inpainted_image, f"flaskApp/inpainted/{image_name}", normalize=True)
